    ...
    const getAllReawrdType = async () => {
      try {
        const { status, data } = await axios.get(
          `${API_BASE_URL}/rewards/get/planName`
        );
        if (status === 200) {
  ...
  return (
    <form onSubmit={handleSubmit} className="px-2 py-6" action="">
        ...
        <div className="flex flex-1 flex-col gap-1">
          ...
          <div className="pb-2">
            ...
            <TextField
              ...
              color="secondary"
              value={`${
                values.expirationDate && values.expirationDate + " Days"
              }`}
              error={
                errors.expirationDate && touched.expirationDate ? true : false
              }
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">
      ...
      <div className="mt-2 flex justify-end">
        <button
          className="w-[20%] h-10 flex justify-center items-center gap-2
